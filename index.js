...
const VhtearKey = 'tempel sini' // beli di 083141727903
const XteamKey = 'tempel sini' // APIKEY XTEAM BELI SENDIRI:V
...
break
//FITUR TAMBAHAN IKY ADS THX NASWARI TEAM
case 'tahta':
				if (!isRegistered) return reply(ind.noregis())
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (isLimit(sender)) return reply(ind.limitend(pusname))
...
case 'cocofun':
            ...
            case 'ytmp42':
                ...
                ikyy.sendMessage(from, buffer, video, { mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek })
                await limitAdd(sender)
                break
case 'ytsearch':
                query = args.join(" ")
                anu = await fetchJson(`http://api.lolhuman.xyz/api/ytsearch?apikey=6b25e69d0ba3dc9447010464&query=${query}`, { method: 'get' })
                teks = '=================\n'
            ...
            case 'ytmp3':
                ...
                anu = await fetchJson(`https://docs-jojo.herokuapp.com/api/ytmp4?url=${args[0]}`, { method: 'get' })
                if (anu.error) return reply(anu.error)
                teks = `*Title* : ${anu.title}\n\n*Size* : ${anu.filesize}\n\nã€Œâ—ã€ PROSES DULU BOSSï¸`
                thumb = await getBuffer(anu.thumb)
                ikyy.sendMessage(from, thumb, image, { quoted: mek, caption: teks })
                buffer = await getBuffer(anu.result)
                ikyy.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek })
                await limitAdd(sender)
                break
            case 'ytmp31':
                if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
...
case 'groupinfo':            noelle.updatePresence(from, Presence.composing)
                if (!isGroup) return reply(mess.only.group)
                ppUrl = await noelle.getProfilePicture(from) // leave empty to get your own
			    buffergbl = await getBuffer(ppUrl)
		        noelle.sendMessage(from, buffergbl, image, {quoted: mek, caption: `*NAME* : ${groupName}\n*MEMBER* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\n*DESK* : ${groupDesc}`})
                break
case 'ceritahorror': 
					if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
...
case 'spamcall':
			...
			call = `${body.slice(11)}`
			reply(`_[❕] Loading_`)
			anu = await fetchJson(`https://meiky-xyz.herokuapp.com/api/call/?nohp=${call}`, {method: 'get'})
			ikyy.sendMessage(from, `${anu.result.logs}`, text, {quoted: mek })
			await limitAdd(sender) 
			break  
				case 'wiki':
                ...
                if (anu.error) return reply(anu.error)
                wikii = `${anu.result}`
                ikyy.sendMessage(from, wikii, text, {quoted: mek })
                await limitAdd(sender) 
                break
               case 'asupan1':
                 if (!isRegistered) return reply( ind.noregis())
...
case 'fbdl':
				...
				reply('Loading......')
                    ...
                    break
                  case 'tiktoknowm':
				    anu = await fetchJson(`https://lolhuman.herokuapp.com/api/tiktok?apikey=${lolhuman}&url=${args[0]}`)
				    buffer = await getBuffer(anu.result.link)
				    teks = `*➸ Name:* ${anu.result.author.username}\n➸ *Title:* ${anu.result.title}\n➸ *Desc:* ${anu.result.description}`
				    ikyy.sendMessage(from, buffer, video, { quoted: mek, caption: teks})
				    break
                    case 'igdl':
				...
				reply(`_[❕] Loading_`)
					...
					memein = await fetchJson(`https://meiky-xyz.herokuapp.com/api/random/meme?apikey=meikyads`)
...
case 'darkneon':
			...
			darkn = body.slice(9)
			reply('[❕] Sedang di proses.........')
			darkne = await getBuffer(`https://meiky-xyz.herokuapp.com/api/textmaker/darkneon/?text=${darkn}`)
			...
			papg = body.slice(6)
			reply('[❕] Sedang di proses.........')
			glass = await getBuffer(`https://meiky-xyz.herokuapp.com/api/textmaker/paperonglass/?text=${papg}`)
...
break
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  anu = await imgbb("3ea1465ef91578a90ee81f7d41c59a1f", owgi)
	  hehe = await getBuffer(`https://videfikri.com/api/textmaker/wanted/?urlgbr=${anu.display_url}&text1=Dicari&text2=${tels}`)
	 ikyy.sendMessage(from, hehe, image, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "Neh...", 'jpegThumbnail': fs.readFileSync('odc.jpg')}}}})
	} else {
	  reply('Jangan tambah kan apapun pada command')
	}
	break
